...
interface EntityAnnotation {
  ...
  entity_id?: string; // if linked to existing entity
  notes?: string;
  start_pos: number;
  end_pos: number;
  color: string;
...
interface ChatViewerProps {
  ...
  onAnnotationAdd?: (annotation: Omit<EntityAnnotation, 'id' | 'color'>) => void;
...
export const ChatViewer: React.FC<ChatViewerProps> = ({
  ...
  const renderAnnotatedContent = (content: string) => {
    ...
    for (const annotation of sortedAnnotations) {
      ...
      const text = annotatedContent.slice(annotation.start_pos, annotation.end_pos);
  ...
  const markdownComponents = {
    p: ({ children }: React.PropsWithChildren) => <p className="mb-4 text-glyph-primary leading-relaxed">{children}</p>,
    h1: ({ children }: React.PropsWithChildren) => <h1 className="text-2xl font-bold text-glyph-bright mb-4">{children}</h1>,
    h2: ({ children }: React.PropsWithChildren) => <h2 className="text-xl font-semibold text-glyph-bright mb-3">{children}</h2>,
    h3: ({ children }: React.PropsWithChildren) => <h3 className="text-lg font-medium text-glyph-bright mb-2">{children}</h3>,
    ...
    ul: ({ children }: React.PropsWithChildren) => <ul className="list-disc list-inside mb-4 text-glyph-primary">{children}</ul>,
    ol: ({ children }: React.PropsWithChildren) => <ol className="list-decimal list-inside mb-4 text-glyph-primary">{children}</ol>,
    ...
    <div className="h-full flex flex-col bg-cosmic-deepest">
      ...
      <div className="flex items-center justify-between p-4 border-b border-cosmic-light border-opacity-20">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-bold text-glyph-bright font-serif">Chat Viewer</h2>
      ...
      <div className="flex-1 flex overflow-hidden">
            ...
            {viewMode === 'chat' ? (
                      ...
                      <div className="flex-shrink-0">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                          ...
                          {message.role === 'user' ? (
                            ...
                            <FaRobot className="text-circuit-magic" size={14} />
                          )}
                        </div>
                      </div>
              ...
              <div className="glass-panel p-6 rounded-lg">
                <pre className="whitespace-pre-wrap text-glyph-primary font-mono text-sm leading-relaxed">
                  <div dangerouslySetInnerHTML={{ __html: renderAnnotatedContent(rawContent) }} />
                </pre>
              </div>
            )}
          </div>
        ...
        {/* Annotations Sidebar */}
        <div className="w-80 border-l border-cosmic-light border-opacity-20 bg-cosmic-deep flex flex-col">
          <div className="flex-1 overflow-y-auto p-4">
            ...
            {annotations.length === 0 ? (
              <div className="text-center text-glyph-accent py-8">
                ...
                <p className="text-xs mt-1">Select text to create annotations</p>
              </div>
            ) : (
                    ...
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <div
                          ...
                          style={{ backgroundColor: ENTITY_COLORS[annotation.entity_type] }}
                        />
      ...
      {showAnnotationModal && selectedText && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="glass-panel border border-cosmic-light rounded-xl p-6 w-96 mx-4">
            ...
            <div className="mb-4">
              <div className="text-sm text-glyph-accent mb-2">Selected Text:</div>
            ...
            <div className="mb-4">
              <label className="block text-sm text-glyph-accent mb-2">Entity Type:</label>
              <select 
            ...
            <div className="mb-6">
              <label className="block text-sm text-glyph-accent mb-2">Notes (optional):</label>
              <textarea
                className="w-full glass-panel text-glyph-primary p-2 rounded focus:outline-none focus:ring-2 focus:ring-flame-blue"
                rows={3}
